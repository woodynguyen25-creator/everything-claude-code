...
export function EquityHeader({ state }: EquityHeaderProps) {
  ...
  return (
    <header className="rounded-2xl border border-white/[0.08] bg-[rgba(12,10,26,0.85)] p-4 shadow-[0_18px_70px_rgba(0,0,0,0.5)] backdrop-blur-xl">
      <div className="grid min-h-[140px] grid-cols-1 gap-4 xl:grid-cols-[0.8fr_1.2fr_1fr]">
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-5">
          <div className="text-[10px] uppercase tracking-[0.28em] text-white/40">Olympus Fund</div>
          <div className="mt-3 font-mono text-4xl font-semibold tracking-wide text-white">{currency(state.current_equity)}</div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.16em] text-white/45">
            <span className="rounded-full border border-white/[0.08] px-2 py-1">{state.open_positions_count} open</span>
            <span className="rounded-full border border-white/[0.08] px-2 py-1">{currency(state.total_unrealized_pnl)} unrealized</span>
          </div>
        </div>
